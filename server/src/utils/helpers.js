...
const exportedMethods = {
  checkString(strVal, varName) {
    ...
    if (strVal.length === 0)
      ...
      throw new Error(
        `${varName} cannot be an empty string or a string with just spaces`
      );
    return strVal;
  },
  ...
  checkName(strVal, varName) {
    ...
    if (!/^[a-zA-Z\s\-']+$/.test(strVal))
      ...
      throw new Error(
        `${varName} must contain only letters, spaces, hyphens, or apostrophes`
      );
    return strVal;
  },
  ...
  checkId(id, varName) {
    ...
    if (!ObjectId.isValid(id))
      throw new Error(`${varName} is an invalid ObjectId`);
    return id;
  },
  ...
  checkDate(dob, varName) {
    ...
    if (parts.length !== 3)
      throw new Error(`Date in ${varName} must be in MM/DD/YYYY format`);
    ...
    if (day < 1 || day > maxDaysInMonth)
      throw new Error(`Day is not valid in ${varName}`);
  ...
  checkStringArray(arr, varName) {
    ...
    return arr.map((item, index) => {
      if (typeof item !== "string" || item.trim().length === 0)
        ...
        throw new Error(
          `Element at index ${index} in ${varName} is not a valid string`
        );
      return item.trim();
    });
  },
  ...
  checkRating(rating, varName) {
    rating = this.checkNumber(rating, "rating");
