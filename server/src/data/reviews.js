...
import * as apartmentFunctions from "./apartments.js";
...
export const approveReviewById = async (id) => {
  ...
  const approvedReview = await getReviewById(id);
  await apartmentFunctions.updateApartmentRatingById(approvedReview.apartmentId);
...
export const getAllReviewsByApartmentId = async (apartmentId) => {
  const reviewCollection = await reviews();
  const reviewList = await reviewCollection
    .find({ apartmentId: apartmentId, isApproved: true })
    .toArray();
  for (let i = 0; i < reviewList.length; i++) {
    formatReviewObject(reviewList[i]);
  }
